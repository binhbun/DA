...
use App\Core\PayOS\Exceptions\ErrorCode;
use App\Core\PayOS\Exceptions\ErrorMessage;
use Exception;
...
class PayOS
{
    ...
    public function createPaymentLink(array $paymentData): array
    {
        ...
        $url = env('PAYOS_BASE_URL') . '/v2/payment-requests';
    ...
    public function getPaymentLinkInformation($orderCode): array
    {
        ...
        $url = env('PAYOS_BASE_URL') . '/v2/payment-requests/' . $orderCode;
        try {
            $headers = array(
                'x-client-id: ' . $this->clientId,
                'x-api-key: ' . $this->apiKey,
    ...
    public function confirmWebhook(string $webhookUrl): string
    {
        ...
        $url = env('PAYOS_BASE_URL') . '/confirm-webhook';
     ...
     * @throws Exception
     */
    public function cancelPaymentLink($orderCode, ?string $cancellationReason = null): array
    {
        if (!$orderCode || (is_string($orderCode) && strlen($orderCode) == 0) || (is_int($orderCode) && $orderCode < 0)) {
            throw new Exception(ErrorMessage::INVALID_PARAMETER, ErrorCode::INVALID_PARAMETER);
        }
        $url = env('PAYOS_BASE_URL') . '/v2/payment-requests/' . $orderCode . '/cancel';
        try {
            $headers = array(
                'x-client-id: ' . $this->clientId,
                'x-api-key: ' . $this->apiKey,
