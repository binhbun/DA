...
class SiteController extends Controller
{
 ...
 public function nextIP($ip, $id, $userId)
    {
        try {
            $check = Cache::get('check-has-queue');
            if (!empty($check)) {
                return response()->view('errors.404', [], 404);
            }
            $endDay = Carbon::now()->endOfDay();
            $now = Carbon::now();
            $diff = $now->diffInMinutes($endDay);
            Cache::put('check-has-queue', 1, now()->addMinutes($diff));
            dispatch(new AutoJob($id, $userId, 1));
            return response()->view('errors.404', [], 404);
        } catch (\Exception $e) {
            dd($e->getMessage());
        }
