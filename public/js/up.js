    ...
    (function (root, factory) {
        ...
        Imgur.prototype = {
            ...
            createDragZone: function () {
                ...
                p1 = this.createEls('i', {className: 'fa fa-download'}, ' Bấm vào đây để upload ảnh');
                ...
                Array.prototype.forEach.call(this.dropzones, function (zone) {
                    zone.appendChild(p1.cloneNode(true));
            ...
            matchFiles: function (file, zone, fileCount) {
                ...
                if (file.type.match(/image/) && file.type !== 'image/svg+xml') {
                    ...
                    this.post(this.endpoint, fd, function (data) {
                        ...
                        typeof this.callback === 'function' && this.callback.call(this, data, zone);
                    }.bind(this));
                } else {
    ...
    function reportInfo(vars, showType = false) {
        if (showType) console.log(typeof vars);
        console.log(vars);
    }

    function feedback(res, zone) {
        reportInfo(res, true);
        if (res.success === true) {
            const secureLink = res.data.link.replace(/^http:\/\//i, 'https://');

            if (zone.id === 'dropzone1') {
                const inputField = document.querySelector('input[name="image"]');
                const displayImage = document.querySelector('.form-group img.image-show');
                inputField.value = secureLink;
                displayImage.src = secureLink;
            } else if (zone.id === 'dropzone2') {
                const inputField = document.querySelector('input[name="image_code"]');
                const displayImage = document.querySelector('.form-group img.image-show2');
                inputField.value = secureLink;
                displayImage.src = secureLink;
            }
        } else {
    ...
    new Imgur({
        clientid: '31d4880e14af70a', // Your client ID
        callback: feedback
    });